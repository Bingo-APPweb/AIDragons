...
[![Node](https://img.shields.io/badge/node-18|20|22-3C873A)](#)
...
- [Detalhes do Caos](#-detalhes-do-caos)
- [Benchmarks](#-benchmarks)
- [Reprodutibilidade](#-reprodutibilidade)
- [Integração BBF-MAP](#-integração-bbf-map)
- [Segurança](#-segurança)
...
```typescript
// src/contracts/NPS_BBFEvent.ts
export type NPS_BBFEvent = {
  spec: "bbf-map/nps-event@1.0.0";
  runId: string;                     // ISO datetime or UUID
  source: "NPS";
  sampleRateHz: number;
  weakSignals: Array<{ 
    freqHz: number; 
...
```bash
# Enviar dados para o BBF-MAP
curl -X POST "$BBF_WEBHOOK_URL" \
  -H "Content-Type: application/json" \
  -H "X-BBF-Signature: sha256=$SIGN" \
  --data @.nps/last-run.json

# Gerar assinatura
SIGN=$(node -e "const c=require('crypto');process.stdout.write(''+c.createHmac('sha256',process.env.BBF_SECRET).update(require('fs').readFileSync('.nps/last-run.json')).digest('hex'))")
```
...
Relate vulnerabilidades para **security@seu-dominio**.
- Janela de resposta: 72h
- Correções críticas: 14 dias
- Não publique PoC com dados reais
- Versões suportadas: últimas 3 releases
...
# CITATION.cff
cff-version: 1.2.0
title: Natural Prediction System
message: If you use this software, please cite it as below.
authors:
  - family-names: Correa
    given-names: Jober Mögele
version: 1.0.0
